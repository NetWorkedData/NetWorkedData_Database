...
namespace SQLite4Unity3d
{
    ...
    public partial class SQLiteConnection : IDisposable
    {
        ...
        public void Close()
        {
            if (this._open && this.Handle != NullHandle)
                ...
                {
                    if (this._mappings != null)
                        foreach (TableMapping sqlInsertCommand in this._mappings.Values)
    ...
    public class TableMapping
    {
        ...
        public PreparedSqlLiteInsertCommand GetInsertCommand(SQLiteConnection conn, string extra)
        {
            ...
            {
                this._insertCommand = CreateInsertCommand(conn, extra);
    ...
    public static class Orm
    {
        ...
        public static string SqlDecl(TableMapping.Column p, bool storeDateTimeAsTicks)
        {
            ...
            if (!p.IsNullable)
            {
                //------------------------------------------
                //---------- ADD IDEMOBI START -------------
                if (p.ColumnType == typeof(Boolean) ||
                    p.ColumnType == typeof(Byte) ||
                    p.ColumnType == typeof(UInt16) ||
                    p.ColumnType == typeof(SByte) ||
                    p.ColumnType == typeof(Int16) ||
                    p.ColumnType == typeof(Int32) ||
                    p.ColumnType == typeof(UInt32) ||
                    p.ColumnType == typeof(Int64) ||
                    p.ColumnType == typeof(Single) ||
                    p.ColumnType == typeof(Double) ||
                    p.ColumnType == typeof(Decimal) ||
                    p.ColumnType == typeof(TimeSpan) ||
                    p.ColumnType == typeof(DateTime) ||
                    p.ColumnType == typeof(DateTimeOffset) ||
                         p.ColumnType.IsSubclassOf(typeof(NWEDataType))
                    )
                {
                    decl += "not null default 0 ";
#if !NETFX_CORE
                }
                else if (p.ColumnType.IsEnum)
                {
#else
                } else if (p.ColumnType.GetTypeInfo().IsEnum) {
#endif
                    decl += "not null default 0 ";
                }
    ...
    public class SQLiteCommand
    {
        ...
        {
            return ExecuteDeferredQuery<T>(this._conn.GetMapping(typeof(T)));
        }

        public List<T> ExecuteQuery<T>()
        {
            return ExecuteDeferredQuery<T>(this._conn.GetMapping(typeof(T))).ToList();
        }
        ...
        public override string ToString()
        {
            string[] parts = new string[1 + this._bindings.Count];
            parts[0] = this.CommandText;
            int i = 1;
            foreach (Binding b in this._bindings)
            {
        ...
        internal static int BindParameter(Sqlite3Statement stmt, int index, object value, bool storeDateTimeAsTicks)
        {
            ...
            //------------------------------------------
            //---------- ADD IDEMOBI START -------------
            if (value.GetType().IsSubclassOf(typeof(NWEDataType)))
            {
                NWEDataType tValue = (NWEDataType)value;
                if (tValue == null)
                {
                    tValue = Activator.CreateInstance(value.GetType()) as NWEDataType;
                }
                return SQLite3.BindText(stmt, index, tValue.ToString(), -1, NegativePointer);
            }
            if (value.GetType().IsSubclassOf(typeof(NWEDataTypeInt)))
            {
                NWEDataTypeInt tValue = (NWEDataTypeInt)value;
                if (tValue == null)
                {
                    tValue = Activator.CreateInstance(value.GetType()) as NWEDataTypeInt;
                }
                return SQLite3.BindInt64(stmt, index, tValue.ToLong());
            }
            if (value.GetType().IsSubclassOf(typeof(NWEDataTypeFloat)))
            {
                NWEDataTypeFloat tValue = (NWEDataTypeFloat)value;
                if (tValue == null)
                {
                    tValue = Activator.CreateInstance(value.GetType()) as NWEDataTypeFloat;
                }
                return SQLite3.BindDouble(stmt, index, tValue.ToDouble());
            }
            if (value.GetType().IsSubclassOf(typeof(NWEDataTypeEnum)))
            {
                NWEDataTypeEnum tValue = (NWEDataTypeEnum)value;
                if (tValue == null)
                {
                    tValue = Activator.CreateInstance(value.GetType()) as NWEDataTypeEnum;
                }
                return SQLite3.BindDouble(stmt, index, tValue.ToLong());
            }
            if (value.GetType().IsSubclassOf(typeof(NWEDataTypeMask)))
            {
                NWEDataTypeMask tValue = (NWEDataTypeMask)value;
                if (tValue == null)
                {
                    tValue = Activator.CreateInstance(value.GetType()) as NWEDataTypeMask;
                }
                return SQLite3.BindDouble(stmt, index, tValue.ToLong());
            }
            //---------- ADD IDEMOBI FINISH ------------
    ...
    public class TableQuery<T> : BaseTableQuery, IEnumerable<T>
    {
        ...
        private SQLiteCommand GenerateCommand(string selectionList)
        {
            ...
            if (this._where != null)
            {
                CompileResult w = CompileExpr(this._where, args);
            ...
            if (this._orderBys != null && this._orderBys.Count > 0)
            {
                string t = string.Join(", ",
                    this._orderBys.Select(o => "\"" + o.ColumnName + "\"" + (o.Ascending ? "" : " desc")).ToArray());
                cmdText += " order by " + t;
            }
